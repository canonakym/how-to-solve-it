...
-   **R**：プログラミング言語の一つ。特にデータ解析に特化している。
-   **RStudio**：Rに特化した統合開発環境(IDE)

:::

では、Rを触ってみましょう！Rはプログラミング言語の一つです。Rをインストールして開いてみると、`R console`が開きます。ただ、この`R.app`の`R console`はあまり使い勝手が良くありません。そこで、Rをより使いやすく、直感的に、お助けツールまで用意してくれているのが**RStudio**です。
...
まずは @lst-calc-basic1 を**コンソール**で直接実行してみましょう。これからたくさんコードが出てきますが、一回は自分の手で打ち込むようにしてください。コピペは楽ですが、打ち込むことでコードの構造を理解できます。

@lst-calc-basic1 を実行したら、書いたコードのすぐ下に`[1] 2`と表示されたと思います。このようにコンソールで実行した結果は、すぐ下に表示されます。では、もう少し複雑な計算をしてみましょう。
...
また、今回手本コード (@calc-basic3) に書いてある`# コメント`の内容は表示だけされて実行結果には影響を与えなかったはずです。

Rに限らずプログラム言語には**コメントアウト機能**が備わっています。そのとき何を考えてそのコードを書いたのか、意外と忘れます。コメントと一緒にスクリプトを書くことでコードの再現性がより高くなります。こまめにコメントアウトをする癖をつけましょう。コメントアウトは`#`を書いてから書き込む方法と選択行を一気にコメントアウトする`⌘⇧C`のどちらかでできます。
...
データ読み込みの前に知っておくべき知識があります。**オブジェクト**です。
...
@lst-object-basic1 では同じデータを何回も書いています。面倒この上ないし、いつかデータをかき間違えることでしょう。では次にオブジェクトを使って同じことをします。
...
@lst-object-basic2 を実行してみましょう。 このコードでは、`c(3, 4, 7, 10, 30)`を `data1`に格納し、`mean()`と`sd()`に**引数**として渡しています。@lst-object-basic1 と同じ結果なのに書く労力が段違いですね！これがオブジェクトを使うメリットです。

このように、`オブジェクト名 <- 何か`という操作を「オブジェクトに値を格納する」と言います。今回の例では、`data1`というオブジェクトにベクトルを格納しています。オブジェクト名だけを実行すると、その中身を確認することもできます。

また、今回平均値を計算する`mean()`と、不偏標準偏差を計算する`sd()`という2つの関数を使いました。関数は、特定の処理を行うための仕組みで、`mean()`や`sd()`のように関数名の後に`()`をつけて呼び出します。`()`の中に入れる値を**引数**と呼び、関数に渡すデータを指定します。

@lst-object-basic1 のようにデータそのものを渡すこともできますが、オブジェクトを渡す方が効率的です。データが変更された場合でも、オブジェクトの中身を変更するだけで済み、コードの修正を最小限に抑えることができます。今回はベクトルを格納しましたが、行列やデータフレームでもなんでもオブジェクトに格納することができます。
...
`tidyverse`に属す関数で読み込むとデータ型(data type)を自動で判断してくれるというメリットもあります。データ型というのは、データがどのような種類であるかを示すものです。例えば、上のコードでIDは数値型`<dbl>`、genotypeは文字列型`<chr>`、motilityは数値型`<dbl>`と自動で判断されていることがわかります。データフレームを作成する際に、列ごとに異なるデータ型を指定することもできますが、`tidyverse`の関数を使用すると、データ型の自動判断が行われるため、手動で指定する必要がなくなります。
...
では次に作業環境を整えましょう。Rでの解析を行う際には、**Rプロジェクト**を作成することをお勧めします。Rプロジェクトは、特定の解析に関連するファイルやデータを整理するためのフォルダ構造を提供し、作業環境を整えるための機能です。Rプロジェクトを作成することで、解析に必要なファイルやデータを一元管理できるようになります。また、Rプロジェクトは、作業ディレクトリを自動的に設定してくれるため、データの読み込みや保存の際にパスを指定する手間が省けます。

Rプロジェクトを作成するには、RStudioのメニューバーから`File → New Project`を選択し、出てくるウィンドウにしたがってプロジェクトを作成してください。このsessionでは、Directory　nameを`R_course`、Create project as a subdirectory ofをDesktopに設定してプロジェクトを作成しましょう。

![](images/source/Rproject.png){fig-align="center"}

Rプロジェクトは解析ごとやプロジェクトごとに作成しておくと便利です。私はRプロジェクトを作成するフォルダには、raw_data、inputs、outputs、scripts、docsなどのサブフォルダを作成しています。


### CSVファイルの読み込み
では、実際にCSVファイルからデータを読み込んでみましょう。デモデータを用意したのでダウンロードしてください。さっき作った`R_course/inputs`の中にダウンロードしたデータを入れましょう。
...
R_courseフォルダに`.Rproj`を作成している場合は、

`inputs/code_csvdata_readr.csv`

と指定するだけでOKです。

これは経験談ですが、フォルダ名やファイル名にスペースがあると、高確率でエラーになります。スペースは視認できない上、全角だったり半角だったりすると地獄です。スペースの代わりにアンダーバー`_`を使うとよいです。
:::
...
`read_csv()`は`readrパッケージ`の関数でcsvをデータフレームとして読み込んでくれます。他にも`read_tsv()`などいろいろな拡張子に対応します。エクセルファイル(.xlsx)の読み込みは`readrパッケージ`では不可能です。`readxlパッケージ`が必要になります。ただ、エクセルファイルはシートの指定などが必要になる上、エラーもよく起こるので、この講座では一貫してCSVファイルを扱います。
...
@lst-visualization1 で作ったグラフの種類・見た目を変えてみよう。さらに、yに与える引数を変えて、体重以外のデータもグラフにしてみよう。
